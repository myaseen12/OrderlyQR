...
-- 1. Profiles
create policy "Users can view profiles if they are self or restaurant co-members"
  ...
  using (
    ...
    or exists (
      ...
      where m1.user_id = profiles.id
        and exists (
          select 1 
          from public.restaurant_members m2
...
create policy "Customers can view their own order status"
  ...
  to public
  using (true); -- Guest access allows scanning by id or showing tables (filtered in application layer by ID/cookie)
